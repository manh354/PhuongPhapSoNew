...
num_step = int(num_step/2)
dataX2 = np.linspace(a,b,num_step + 1)
dataY2 = []
for i in range(len(dataX)):
    if i%2==0:
        dataY2.append(dataY[i])
simpsonsY2 = [4*y if i%2 != 0 else 2*y for (i,y) in enumerate(dataY2)]
simpsonsY2[0] = dataY2[0]
simpsonsY2[-1] = dataY2[-1]
integrated_result2 =  np.multiply(h/3,np.sum(simpsonsY2))
print("Giá trị tích phân với grid = {0} là: ".format(num_step),integrated_result2)
