...
def deEulerForward(symbolic_function_system : list, symbolic_vars : list[sp.Symbol], symbolic_t : sp.Symbol , vars_start: list[float], t_start: float, t_end: float,h : float):
    list_result_t = []
    list_result_vars = []
    ...
    while t_iterate < t_end:
        vars_iterate = eulerForwardPredictor(lamdified_equation_system,vars_iterate,t_iterate,h)
        t_iterate += h
        list_result_t.append(t_iterate)
        list_result_vars.append(vars_iterate)
    return list_result_t,list_result_vars

def eulerForwardPredictor(lamdified_equation_system, vars_iterate,t_iterate,h):
