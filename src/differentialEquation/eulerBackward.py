...
def deEulerBackward(symbolic_equation_system : list, symbolic_vars : list[sp.Symbol], symbolic_t : sp.Symbol , vars_start: list[float], t_start: float, t_end: float,h : float):
    list_result_t = []
    list_result_vars = []
...
def fixedpointIteration(lamdified_equation_system ,vars_start : list, t_start, h : float ,epsilon, terminate_threshold):
    vars_iterate = vars_start.copy()
    t_iterate = t_start
    equation_system_value = [equation((vars_iterate),t_iterate + h) for equation in lamdified_equation_system]
...
def eulerBackwardCorrector(lamdified_equation_system ,vars_start : list, t_start, h : float ,EPS, ITR_MAX):
    vars_iterate = vars_start.copy()
    t_iterate = t_start
    equation_system_value = [equation((vars_iterate),t_iterate + h) for equation in lamdified_equation_system]
    vars_iterate_new = np.add(vars_start,np.multiply(h, equation_system_value))
