...
def makeAMWorks(number_of_points_used, symbolic_function_system : list, symbolic_vars : list[sp.Symbol], symbolic_t : sp.Symbol , multiple_points_vars_start: list[list[float]], multiple_points_t_start: list[float], t_end: float,h : float):
    ...
    if(number_of_points_used < len(multiple_points_t_start)):
        ...
        return mainRungeKutta4_Classic(symbolic_function_system,symbolic_vars,symbolic_t,multiple_points_vars_start[position],multiple_points_t_start[position],multiple_points_t_start[position]+(number_of_points_used - position -1 + 0.1)*h, h)
